...
//stream
// export const LoLa_Reader = zig_std.Io.Reader;
// fn readerFromMem(mem: [*]u8, len: usize) LoLa_Reader {
//     return LoLa_Reader.fixed(mem[0..len]);
// }
var global_error: ?anyerror = null;

export fn LoLa_getError() [*:0]const u8 {
    if (global_error) |err| {
...
export fn loadCUFromMem(mem: [*]u8, len: usize) ?*CompileUnit {
    var reader = zig_std.Io.Reader.fixed(mem[0..len]);
    const alloc = zig_std.heap.c_allocator;
