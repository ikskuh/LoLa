...
namespace LoLa::Compiler
{
    ...
    struct CodeWriter
    {
        ...
        void emitBreak(ErrorCollection &errors);
        void emitContinue(ErrorCollection &errors);
    };
    ...
    struct ErrorCollection
    {
        ...
        void add(CompileError &&error);

        void invalidStore(std::string const &str);

        void invalidVariable(std::string const &str);

        void variableNotFound(std::string const &str);

        void invalidString(std::string const &str);

        void invalidOperator(AST::Operator op);
    ...
    struct Compiler
    {
        ...
        std::shared_ptr<CompilationUnit> compile(AST::Program const &program);
    };
} // namespace LoLa::Compiler
